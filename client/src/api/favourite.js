...
export const addBookToFavourite = async (bookId) => {
  const response = await api.put('/favourite/add-book-to-favourite', {}, { headers: { bookid: bookId } });
  return response.data;
};

export const removeBookFromFavourite = async (bookId) => {
  const response = await api.put('/favourite/remove-book-from-favourite', {}, { headers: { bookid: bookId } });
  return response.data;
};
